...
export const colors = {
  ...
  modeReading: 'var(--roam-primary-color, #8cb4ff)',  // Read 模式 = 蓝色 = 与 primary intent 一致
...
Buttons (primary/success/warning/danger)
    ↓ 使用模式颜色
ModeBadge (success=Spaced / warning=Fixed / primary=Read)
...
### Q: modeReading 为什么用蓝色？
A: 蓝色对应 Roam 的 primary intent 颜色，与绿色（Spaced/记忆）和橙色（Fixed/复习）形成三色区分体系，语义上蓝色代表"阅读/理解"与绿色"记忆"和橙色"复习"互补。
